<div class="space-y-6" x-data="{ showModal: @entangle('showModal'), showDeleteModal: @entangle('showDeleteModal') }">
    ...
    <div class="relative overflow-hidden rounded-3xl bg-slate-900 border border-slate-800 p-8 mb-8">
        ...
        <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                ...
                <p class="text-slate-400">Manage medical professionals and clinical practitioners</p>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6">
            <p class="text-slate-500 text-xs font-bold uppercase tracking-widest mb-2">Total Doctors</p>
            <p class="text-3xl font-black text-white">{{ $doctors->total() }}</p>
        </div>
        <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6">
            <p class="text-slate-500 text-xs font-bold uppercase tracking-widest mb-2">Active</p>
            <p class="text-3xl font-black text-emerald-500">{{ $doctors->where('is_active', true)->count() }}</p>
        </div>
    </div>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-[2.5rem] overflow-hidden">
        <div class="p-8 border-b border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h3 class="text-xl font-bold text-white">Medical Practitioners</h3>
            <div class="relative">
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search by name or ID..." class="w-full md:w-80 pl-10 pr-4 py-2.5 bg-slate-800 border-slate-700 rounded-xl text-sm text-white focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-800">
                        ...
                        <tr class="hover:bg-slate-800/20 transition-colors group">
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-cyan-500 to-violet-500 p-0.5">
                                        <div class="w-full h-full bg-slate-900 rounded-[0.9rem] flex items-center justify-center">
                                            <span class="text-white font-bold">{{ substr($doc->user->name, 0, 1) }}</span>
                                        </div>
                                    </div>
                                    <div>
                                        <p class="text-white font-semibold">Dr. {{ $doc->user->name }}</p>
                                        <p class="text-slate-500 text-xs tracking-tight">{{ $doc->employee_id }} • {{ $doc->qualification }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                                <span class="px-3 py-1 bg-violet-500/10 text-violet-400 rounded-lg text-xs font-bold">{{ $doc->department?->name ?? 'Unassigned' }}</span>
                            </td>
                            <td class="px-8 py-6 text-emerald-500 font-bold">
                                ${{ number_format($doc->consultation_fee, 2) }}
                            </td>
                            <td class="px-8 py-6">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold {{ $doc->is_active ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-800 text-slate-500' }}">
                                    {{ $doc->is_active ? 'On Duty' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-8 py-6 text-right">
                                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button wire:click="edit({{ $doc->id }})" class="p-2 bg-slate-800 text-slate-400 hover:text-cyan-400 rounded-xl transition-colors border border-slate-700">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </button>
                                    <button wire:click="confirmDelete({{ $doc->id }})" class="p-2 bg-slate-800 text-slate-400 hover:text-rose-400 rounded-xl transition-colors border border-slate-700">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-8 py-12 text-center text-slate-400">No doctors registered</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-show="showModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;" x-cloak>
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div x-show="showModal" x-transition.scale.95 class="relative bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 max-w-2xl w-full shadow-2xl">
                <h3 class="text-2xl font-bold text-white mb-6">{{ $staffId ? 'Edit' : 'Add' }} Doctor</h3>
                
                <form wire:submit="save" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Account Details -->
                        <div class="md:col-span-2"><h4 class="text-xs font-black uppercase tracking-[0.2em] text-cyan-500 mb-4">Account Details</h4></div>
                        ...
                        <div class="space-y-2">
                            <label class="text-sm font-medium text-slate-400 ml-1">Password {{ $staffId ? '(Leave blank to keep current)' : '' }}</label>
                            <input type="password" wire:model="password" class="w-full bg-slate-800 border-slate-700 rounded-2xl text-white px-4 py-3 focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all">
                            @error('password') <span class="text-xs text-rose-500 mt-1 ml-1">{{ $message }}</span> @enderror
                        </div>

                        <!-- Professional Details -->
                        <div class="md:col-span-2 mt-4"><h4 class="text-xs font-black uppercase tracking-[0.2em] text-cyan-500 mb-4">Professional Details</h4></div>

                        <div class="space-y-2">
                            <label class="text-sm font-medium text-slate-400 ml-1">Employee ID</label>
                            <input type="text" wire:model="employee_id" class="w-full bg-slate-800 border-slate-700 rounded-2xl text-white px-4 py-3 focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all" placeholder="e.g. DOC-001">
                        ...
                        <div class="space-y-2">
                            ...
                            <select wire:model="department_id" class="w-full bg-slate-800 border-slate-700 rounded-2xl text-white px-4 py-3 focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" wire:model="qualification" class="w-full bg-slate-800 border-slate-700 rounded-2xl text-white px-4 py-3 focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all" placeholder="e.g. MBBS, MD">
                            @error('qualification') <span class="text-xs text-rose-500 mt-1 ml-1">{{ $message }}</span> @enderror
                        </div>

                        <div class="space-y-2">
                            <label class="text-sm font-medium text-slate-400 ml-1">License Number</label>
                            <input type="text" wire:model="license_number" class="w-full bg-slate-800 border-slate-700 rounded-2xl text-white px-4 py-3 focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all">
                            @error('license_number') <span class="text-xs text-rose-500 mt-1 ml-1">{{ $message }}</span> @enderror
                        </div>

                        <div class="space-y-2">
                            <label class="text-sm font-medium text-slate-400 ml-1">Consultation Fee ($)</label>
                            <input type="number" step="0.01" wire:model="consultation_fee" class="w-full bg-slate-800 border-slate-700 rounded-2xl text-white px-4 py-3 focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all">
                            @error('consultation_fee') <span class="text-xs text-rose-500 mt-1 ml-1">{{ $message }}</span> @enderror
                        </div>
                    ...
                    <div class="flex gap-4 pt-4">
                        ...
                        <button type="submit" class="flex-1 px-6 py-4 bg-gradient-to-r from-cyan-600 to-violet-600 hover:from-cyan-500 hover:to-violet-500 text-white rounded-2xl font-bold transition-all shadow-xl shadow-cyan-500/20 active:scale-95">Save Profile</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div x-show="showDeleteModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;" x-cloak>
        <div class="flex items-center justify-center min-h-screen p-4">
            ...
            <div x-show="showDeleteModal" x-transition.scale.95 class="relative bg-slate-900 border border-slate-800 rounded-3xl p-8 max-w-md w-full text-center">
                ...
                <h3 class="text-xl font-bold text-white mb-2">Remove Doctor?</h3>
                <p class="text-slate-400 mb-8 text-sm">Are you sure you want to remove this doctor from the clinic? Their medical records will be preserved.</p>
